...
const ProductList = () => {
  ...
  useEffect(()=>{
    const fetchData=async()=>{
      let data=await dispatch(fetchProduct());
      console.log(state,'state');
      console.log(data,'data');
      MonitorData=state.product.ProductData;
      //console.log('data',data);
      //setMonitorData(data?.payload?.data);
  ...
  if(state.product.isLoading) return;
        ...
        <RightContainer>
            {
              sortedData.map(monitor=>{
                return <ProductView  MonitorData={monitor}/>
              })
            }
        </RightContainer>
      </Conatainer>
